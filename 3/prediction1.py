...
def preprocess_market_data(market_df):
    ...
    # Convert date
    pune_df['Arrival_Date'] = pd.to_datetime(pune_df['Arrival_Date'],dayfirst=True)
...
def create_features(merged_df):
    ...
    # Weather condition trends (7-day change)
    featured_df['Temp_7d_Change'] = featured_df['Temperature'] - featured_df['Temperature'].shift(7)
    featured_df['Humid_7d_Change'] = featured_df['Humidity'] - featured_df['Humidity'].shift(7)
    
    # Interaction between weather and growing conditions
    featured_df['Grow_Index'] = (featured_df['Temperature'] * featured_df['Humidity'] * 
                               featured_df['GWETTOP'] / featured_df['WS2M'].clip(lower=1))
    
    # One-hot encode categorical variables
    # Check column names before calling get_dummies()
    print("Columns before encoding:", merged_df.columns)

    # Strip any leading/trailing spaces from column names (if needed)
    merged_df['Commodity_Original'] = merged_df['Commodity']
    merged_df['Variety_Original'] = merged_df['Variety']

    merged_df.columns = merged_df.columns.str.strip()

    # Ensure 'Season' column exists
  # Rename 'Season_x' or 'Season_y' to 'Season'
    if 'Season_x' in merged_df.columns:
        merged_df.rename(columns={'Season_x': 'Season'}, inplace=True)
    elif 'Season_y' in merged_df.columns:
        merged_df.rename(columns={'Season_y': 'Season'}, inplace=True)

    # Now check again
    print("Columns after renaming:", merged_df.columns)

    # Apply encoding
    featured_df = pd.get_dummies(merged_df, columns=['Season', 'Market', 'Commodity', 'Variety', 'Grade'],drop_first=False)

    print("Columns after encoding:", featured_df.columns)
...
def prepare_lstm_data(df, commodity, variety, sequence_length=30):
   ...
   crop_df = df[(df['Commodity_Original'] == 'Commodity') & (df['Variety_Original'] == 'Variety')].copy()
...
def build_prophet_model(df, commodity, variety):
    ...
    crop_df = df[(df['Commodity_Original'] == Commodity) & (df['Variety_Original'] == Cariety)].copy()
...
def main():
    ...
    top_commodities = featured_df.groupby(['Commodity_Original', 'Variety_Original']).size().sort_values(ascending=False).head(5)
